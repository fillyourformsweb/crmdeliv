...
def calculate_order_amount(weight, billing_pattern_id=None, state=None, client_id=None):
...
def add_client():
    if request.method == 'POST':
        client = Client(
            ...
            gst_number=request.form.get('gst_number'),
            vill_pattern=request.form.get('vill_pattern')
        )
...
def edit_client(id):
    ...
    if request.method == 'POST':
        ...
        client.vill_pattern = request.form.get('vill_pattern')
...
def edit_receiver(id):
    ...
    if request.method == 'POST':
        ...
        receiver.pincode = request.form.get('pincode')
        receiver.gst_number = request.form.get('gst_number')
        receiver.vill_pattern = request.form.get('vill_pattern')
...
def walkin_order():
    if request.method == 'POST':
        ...
        if receipt_type == 'standard':
            # Check if user manually modified it or if we should use the system one
            predicted_number = get_next_receipt_number(current_user)
            if receipt_number == predicted_number:
                # Use standard generator to increment sequence
                receipt_number, assignment_id = generate_receipt_number(current_user)
            else:
                # Treat as manual even if labeled standard because it was edited
                pass 
        ...
        # For walk-in orders, use direct amount input (which will be auto-filled by JS)
        ...
        order.total_amount = received_amount  # For walk-in, total = received
        order.insured_amount = insured_amount
        
        # Populate pricing components if billing pattern is available
        if order.weight and order.billing_pattern_id:
            base, weight_charge, additional, discount, _, _ = calculate_order_amount(
                order.weight, order.billing_pattern_id, state=order.receiver_state
            )
            order.base_amount = base
            order.weight_charges = weight_charge
            order.additional_charges = additional
            order.discount = discount
            # Note: We keep order.total_amount as received_amount because 
            # the user might have overridden it in the form.
        else:
    ...
    return render_template('walkin_order.html')
...
def walkin_bill(id):
    ...
    # Get branch information for "Bill By"
    branch = order.branch
    if not branch:
        branch = Branch.query.get(current_user.branch_id) if current_user.branch_id else Branch.query.first()
        
    return render_template('walkin_bill.html', order=order, branch=branch, is_gst=is_gst)
...
def edit_order(id):
    ...
    if request.method == 'POST':
        ...
        if new_receipt_number != order.receipt_number:
            ...
            if existing_order:
                flash(f'Receipt number {new_receipt_number} already exists in another order! Please use a different number.', 'danger')
        ...
        # Add handling tags logic
        custom_tag = request.form.get('custom_handling_tag', '').strip()
        predefined_tags = request.form.getlist('handling_tags')
        all_tags = predefined_tags.copy()
        if custom_tag:
            all_tags.append(custom_tag)
        order.handling_tags = ','.join(all_tags)
        
        # Add insured amount
        order.insured_amount = float(request.form.get('insured_amount') or 0)
...
def customer_form(token):
    ...
    if request.method == 'POST':
        ...
        order.receiver_pincode = request.form.get('receiver_pincode') or order.receiver_pincode
...
def api_calculate_amount():
    ...
    if weight:
        base, weight_charge, additional, discount, total, rate_type = calculate_order_amount(
            weight, None, state=state, client_id=client_id
        )
        return jsonify({
            'base_amount': base,
...
def admin_receipts():
    if request.method == 'POST':
        ...
        db.session.commit()
        flash('Receipt assignment created successfully!', 'success')
        return redirect(url_for('admin_receipts'))
...
def edit_receipt_assignment(id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash('Receipt assignment updated successfully!', 'success')
        return redirect(url_for('admin_receipts'))
...
def print_consolidated_bill(client_id):
    ...
    if is_gst:
        taxable_value = grand_total_amount
        cgst_amount = taxable_value * 0.09
        sgst_amount = taxable_value * 0.09
        # Update grand total to include GST
        grand_total_amount = taxable_value + cgst_amount + sgst_amount
    else:
        taxable_value = 0
        cgst_amount = 0
        sgst_amount = 0
    
    # Recalculate due amount based on updated grand total (if GST was added)
    grand_due_amount = grand_total_amount - grand_received_amount
